...
def get_summarize_status():
    #fetch job key for summarisation 
    try:
        job_key = request.args.get('job_key')
    except Exception as e:
        return jsonify({"error", e}), 400

    cached = supabase_service.get_cached_summary(job_key)
    if cached:
        #replace with parsed components instead of raw response
        return jsonify({
            "status": "completed",
            "summary": cached.get('summary_text')
        })

    in_flight = supabase_service.get_in_flight_job(job_key)
    if in_flight:
